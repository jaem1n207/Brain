...
## Binary Heap (바이너리 힙)
자료구조의 일종으로 Tree의 형식을 취하고 있으며, Tree 중에서도 배열에 기반한 완전 이진 트리(트리의 마지막 단계에서 오른쪽 부분을 뺀 나머지 부분이 가득 채워져 있기 때문)입니다. 배열에 트리의 값들을 넣어줄 때, 0번째는 건너뛰고 1번 index로부터 Root 노드가 시작됩니다. 이는 노드의 고유번호 값과 배열의 index를 일치시켜 혼동을 줄이기 위함입니다. 힙은 `최대힙`, `최소힙`의 두 가지 형태 중 하나로 제공됩니다.
