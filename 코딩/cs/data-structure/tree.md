...
그럼 위에서 설명한 데이터 구조를 어떻게 생성하고 유지하는 최소힙을 구현할 수 있을까요? 삽입을 예로 들어 보겠습니다. 요소를 삽입하면 항상 위에서 아래로 왼쪽에서 오른쪽으로 보면서 다음 빈 자리로 이동합니다. 아래 사진을 보겠습니다. 요소를 삽입하면 빈 동그라미처럼 왼쪽에서 오른쪽으로 채워집니다. 이게 삽입이 동작하는 방식인데요.
